...
var shuffleDeck = function(deck) {
    ...
    for(var i =0; i < deck.length; i ++) {
        //set random index that is greater than current i, which will disregard already shuffled cards
        let randomIndex = i + Math.floor(Math.random() * (deck.length - i));
        //set alias for current iteration value
        let temp = deck[i];
        //set deck[i] to deck[randomindex]
        deck[i] = deck[randomIndex];
        //set deck[random index] to temp
